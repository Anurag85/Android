cordova.define("cordova-plugin-bbd-storage.FileEntry", function(require, exports, module) {
/*
...
(function() {
    ...
    GDFileEntry.prototype.file = function(successCallback, errorCallback) {
        var win = typeof successCallback !== 'function' ? null : function(f) {
            var file;
            if (f.type === 'null') {
                file = new File(f.name, f.fullPath, null, f.lastModifiedDate, f.size);
            } else {
                file = new File(f.name, f.fullPath, f.type, f.lastModifiedDate, f.size);
            }
                ...
                if (entry) {
                    if (typeof successCallback === 'function') {
                        ...
                        var fs = new(require('cordova-plugin-bbd-storage.FileSystem'))('GDFileSystem'),
                            result = new GDFileEntry(entry.name, entry.fullPath, fs);
    ...
    for (protoFunction in GDFileEntry.prototype) {
        if (GDFileEntry.prototype.hasOwnProperty(protoFunction)) {
            ...
            var objProtoProperty = GDFileEntry.prototype[protoFunction],
