        ...
        var create = options.create ? "true" : "false",
            win = typeof successCallback !== 'function' ? null : function(result) {
                var fs = new(require('cordova-plugin-bbd-storage.FileSystem'))('GDFileSystem'),
                    entry = new(require('cordova-plugin-bbd-storage.DirectoryEntry'))(result.name, result.fullPath, fs);
            ...
            success = function(entry) {
                if (entry) {
                    if (typeof successCallback === 'function') {
                        ...
                        var fs = new(require('cordova-plugin-bbd-storage.FileSystem'))('GDFileSystem'),
                            result = new(require('cordova-plugin-bbd-storage.DirectoryEntry'))(entry.name, entry.fullPath, fs);
            ...
            success = function(entry) {
                if (entry) {
                    if (typeof successCallback === 'function') {
                        // create appropriate Entry object
                        var fs = new(require('cordova-plugin-bbd-storage.FileSystem'))('GDFileSystem'),
                            result = (entry.isDirectory) ? new(require('cordova-plugin-bbd-storage.DirectoryEntry'))(entry.name, entry.fullPath, fs) :
