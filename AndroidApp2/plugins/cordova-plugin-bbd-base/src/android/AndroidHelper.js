...
function ManifestHelper(androidManifestPath) {
    try {
        ...
        this.doc = xml.parseElementtreeSync(androidManifestPath);
    } catch (e) {
        console.log('Parsing ' + androidManifestPath + ' faild');
        throw e;
    }

    this.bbdAndroidManifestValues = [
        { location: './application', attrName: 'android:name', value: 'com.good.gd.cordova.core.GDCordovaApp' },
        { location: './application', attrName: 'android:icon', value: '@drawable/com_bbd_default_logo' },
        { location: './application', attrName: 'android:fullBackupContent', value: '@xml/gd_backup_scheme' },
        { location: './application/activity', attrName: 'android:alwaysRetainTaskState', value: 'true' },
        { location: './application', attrName: 'android:supportsRtl', value: 'false' }
    ];
    this.defaultAndroidManifestValues = [
        { location: './application', attrName: 'android:icon', value: '@drawable/icon' },
        { location: './application', attrName: 'android:supportsRtl', value: 'true' }
    ];
    this.customAndroidManifestValues = {
        icon: { location: './application', attrName: 'android:icon', value: '@mipmap/icon' }
    };
    this.destroyDefaultAndroidManifetsAttr = [
        { location: './application', attrName: 'android:name' },
        { location: './application/activity', attrName: 'android:alwaysRetainTaskState' },
        { location: './application', attrName: 'android:fullBackupContent' }
    ];
}
...
ManifestHelper.prototype = {
    ...
    setAttribute: function(attrObject) {
        if (attrObject.constructor.name !== 'Function')
            this.doc.getroot().find(attrObject.location).attrib[attrObject.attrName] = attrObject.value;
    },
    writeSync: function() {
        fs.writeFileSync(this.path, this.doc.write({ indent: 4 }), 'utf-8');
    },
    editManifest: function() {
        for (attr in this.bbdAndroidManifestValues) {
            var attrObj = this.bbdAndroidManifestValues[attr];
    ...
    restoreManifest: function() {
        for (attr in this.defaultAndroidManifestValues) {
            var attrObj = this.defaultAndroidManifestValues[attr];
            this.setAttribute(attrObj);
        }

        for (attr in this.destroyDefaultAndroidManifetsAttr) {
            var attrObj = this.destroyDefaultAndroidManifetsAttr[attr];
            this.destroyAttribute(attrObj);
        }
...
function ConfigHelper(configXMLPath) {
    try {
        ...
        this.ConfigParser = require('cordova-common').ConfigParser;
        this.configXmlObj = new this.ConfigParser(configXMLPath);
    } catch (e) {
...
function SettingsJsonHelper(context) {
    try {
        this.androidSettingsJsonPath = path.join(context.opts.projectRoot, 'platforms', 'android', 'assets', 'settings.json');
